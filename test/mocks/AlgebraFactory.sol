...
contract AlgebraFactory is IAlgebraFactory {
        ...
        returns (address pool)
    {
        ...
        IDataStorageOperator dataStorage = new DataStorageOperator(
            computeAddress(token0, token1)
        );
    ...
    function setBaseFeeConfiguration(
        ...
        require(
            uint256(alpha1) + uint256(alpha2) + uint256(baseFee) <=
                type(uint16).max,
            "Max fee exceeded"
        );
        require(
            gamma1 != 0 && gamma2 != 0 && volumeGamma != 0,
            "Gammas must be > 0"
        );
    ...
    bytes32 internal constant POOL_INIT_CODE_HASH =
        0x5bee37621da1039b303e51c71fc04f57226ebc52067c9e8adb33b89ac91621d8;

    /// @notice Deterministically computes the pool address given the factory and PoolKey
    /// @param token0 first token
    /// @param token1 second token
    /// @return pool The contract address of the Algebra pool
    function computeAddress(address token0, address token1)
        internal
        view
        returns (address pool)
    {
